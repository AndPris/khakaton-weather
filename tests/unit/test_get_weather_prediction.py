...
class TestWeatherPrediction(asynctest.TestCase):
    ...
    async def test_get_weather_prediction_data_positive(self, mocked_get):
        ...
        response = client.get("/api/v1/get_weather_prediction?lat=42.0&lon=24.0")

        self.assertEqual(response.status_code, 200)

        response_data = response.json()
        response_date = [
            datetime.datetime.fromisoformat(date) for date in response_data["date"]
        ]
    ...
    async def test_get_weather_prediction_data_negative(self, mocked_get):
        ...
        self.assertEqual(response.json(), {"message": "Check your API key or params"})
